...
class UserManager
{
    ...
    public function addUser($data) 
    {
        ...
        if($this->checkUserExists($data['email'])) {
            throw new \Exception("Usuario con dirección de correo electrónico " . $data['$email'] . " ya existe");
        }
    ...
    public function updateUser($user, $data) 
    {
        ...
        if($user->getEmail()!=$data['email'] && $this->checkUserExists($data['email'])) {
            throw new \Exception("Otro usuario con direccion de correo electronico " . $data['email'] . " ya existe");
        }
    ...
    private function assignRoles($user, $roleIds)
    {
        ...
        foreach ($roleIds as $roleId) {
            $role = $this->entityManager->getRepository(Role::class)
                    ->find($roleId);
            if ($role==null) {
                throw new \Exception('Rol no encontrado por ID');
            }
    ...
    public function generatePasswordResetToken($user)
    {
        if ($user->getStatus() != User::STATUS_ACTIVE) {
            throw new \Exception('No se puede generar el token de restablecimiento de contraseña para el usuario inactivo ' . $user->getEmail());
        }
        ...
        $mail->setBody($body);
        $mail->setFrom('no-reply@myglobalux.com', 'MyGlobal');
