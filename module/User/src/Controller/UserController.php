...
class UserController extends AbstractActionController 
{
            ...
            if($form->isValid()) {
                ...
                if (!$this->userManager->changePassword($user, $data)) {
                    $this->flashMessenger()->addErrorMessage(
                            'Lo sentimos, la contraseña anterior es incorrecta. No se pudo establecer la nueva contraseña.');
                } else {
                    $this->flashMessenger()->addSuccessMessage(
                            'Cambió la contraseña con éxito.');
                }
    ...
    public function messageAction() 
    {
        ...
        if($id!='invalid-email' && $id!='sent' && $id!='set' && $id!='failed') {
            throw new \Exception('Se ha especificado un ID de mensaje no válido');
        }
    ...
    public function setPasswordAction()
    {
        ...
        if ($token!=null && (!is_string($token) || strlen($token)!=32)) {
            throw new \Exception('Tipo o longitud de token no válido');
        }
